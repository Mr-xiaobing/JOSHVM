...
public class AMSCommandDispacher implements Runnable {
	...
	public void run() {
		try {
			while (!appmanager.isStopped()) {
			  try {
				...
				if ((server_cmd != null) && (cmdListener != null)) {
					System.out.println("Received message id:"+server_cmd.typeID());
					...
					case AMSMessage.MESSAGE_TYPE_ID_DOWNLOAD_APP:
						if (appmanager.isConnected()) {
							...
							System.out.println("installSource="+installSource);
							System.out.println("mainclass="+mainclass);
							System.out.println("appname="+appname);
							System.out.println("autostart="+autostart);
							System.out.println("startAfterInstall="+startAfterInstall);
					...
					case AMSMessage.MESSAGE_TYPE_ID_RESET_JVM:
						...
						break;
					default:
						System.out.println("AMS received unknown message: "+server_cmd.type());
					}
				}
			...
			e.printStackTrace();
			System.out.println("Exception caught in AMS Message Dispatcher Loop");
		}
